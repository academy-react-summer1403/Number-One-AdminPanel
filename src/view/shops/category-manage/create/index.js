...
const AddShopCategoryModal = ({
  ...
  const { mutate: AddCategory } = useMutation({
    mutationKey: ["CREATE_SHOP_CATEGORY"],
    mutationFn: (values) => {
      CreateShopCategory(values, refetch);
    },
  ...
  const { mutate: updateMutate } = useMutation({
    ...
    mutationFn: (values) => {
      UpdateShopCategory(categoryDetails.id, values, refetch);
    },
    onSuccess: () => setShowModal(!showModal),
  });
  // initialValues
